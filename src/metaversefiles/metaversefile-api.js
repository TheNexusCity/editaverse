...
function createPointerEvents(store) {
  ...
  return {
    connected: false,
    handlers: Object.keys(names).reduce((acc, key) => ({ ...acc, [key]: handlePointer(key) }), {}),
    connect: target => {
...
metaversefile.setApi({
  ...
  usePhysics() {
    ...
    if (app) {
      ...
      const localMatrix = new THREE.Matrix4(); */
      // const localMatrix2 = new THREE.Matrix4();
      physics.addBoxGeometry = (addBoxGeometry =>
      ...
      physics.addCapsuleGeometry = (addCapsuleGeometry =>
        function(position, quaternion, radius, halfHeight, physicsMaterial, dynamic, flags) {
          // const basePosition = position;
          // const baseQuaternion = quaternion;
      ...
      physics.addGeometry = (addGeometry =>
        function(mesh) {
      ...
      physics.addCookedGeometry = (addCookedGeometry =>
      ...
      physics.addConvexGeometry = (addConvexGeometry =>
        function(mesh) {
      ...
      physics.addCookedConvexGeometry = (addCookedConvexGeometry =>
        function(buffer, position, quaternion, scale) {
      ...
      physics.removeGeometry = (removeGeometry =>
...
`;
  ...
  getAppByInstanceId(instanceId) {
    let result =
      world.appManager.getAppByInstanceId(instanceId) || localPlayer.appManager.getAppByInstanceId(instanceId);
    if (result) {
      return result;
    } else {
  ...
  getPhysicsObjectByPhysicsId(physicsId) {
    let result =
      world.appManager.getPhysicsObjectByPhysicsId(physicsId) ||
